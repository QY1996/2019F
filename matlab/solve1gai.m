...
success_types = data1(2:pointCounts-1, 6);
...
for it = 1:100
            ...
            if (~all(now_point == error_points(i, :)) && flag)
                if (error_types(i) == 1)
                    if (norm(now_point - error_points(i, :)) * delta + now_error(1) < a1 && norm(now_point - error_points(i, :)) * delta + now_error(2) < a2)
                        ...
                        available_success = [available_success success_types(i)];
    ...
    %         计算紧张度
            ...
            suits(i) = (p * available_anglescos(i) * 10000 + (1 - p) * available_lengths(i)) + 1e5 / tense;
            if (available_success(i) == 1)
                suits(i) = suits(i) / 1e10;
            end
    ...
    if (step < best_steps)
        best_steps = step;
        best_length = length;
        best_road = road_point;
        disp('good');
        best_p = p;
    elseif(step == best_steps && length < best_length)
        best_length = length;
        best_road = road_point;
        disp('good');
        best_p = p;
...
index = [];
for i = 1:size(best_road, 1)
    for j = 1:size(data1, 1)
        if (all(best_road(i, :) == data1(j, 2:4)))
            index = [index data1(j, 1)];
            break;
        end
    end
end
